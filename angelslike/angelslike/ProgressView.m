...
- (void)addArcBackColor{
    CGColorRef color = (_arcBackColor == nil) ? [UIColor grayColor].CGColor : _arcBackColor.CGColor;
...
- (void)drawArc{
    ...
    if (_percent == 1) {
        ...
        CGContextBeginPath(contextRef);
        CGContextMoveToPoint(contextRef, center.x, center.y);
        CGContextAddArc(contextRef, center.x, center.y, radius, - M_PI / 2,endAngle - M_PI / 2, 0);
        CGContextSetFillColorWithColor(contextRef, color);
        CGContextFillPath(contextRef);
    }
