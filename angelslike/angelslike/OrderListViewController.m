...
-(void)cellClick:(UIButton *)b{
    ...
    if (b.tag == OrderCellType_Detail) {
        ...
        PayNowViewController *vc = [[PayNowViewController alloc]init];
        vc.orderid = [self.result[indexPath.row] strForKey:@"orderid"];
        vc.price  = [self.result[indexPath.row] strForKey:@"price"];
        [self.navigationController pushViewController:vc animated:YES];
